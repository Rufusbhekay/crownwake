...
test("patrol cohesion sends only distant stragglers back to their group", () => {
  const desired = { x: 4, z: 0 }, center = { x: 0, z: 0 };
  assert.deepEqual(patrolCohesionTarget({ unit: { x: 1.8, z: 0 }, center, desired, radius: 2.4 }), desired);
  assert.deepEqual(patrolCohesionTarget({ unit: { x: 2.5, z: 0 }, center, desired, radius: 2.4 }), center);
});

test("independent commander movement preserves soldiers already committed to a duel", () => {
  const target = { userData: { alive: true } };
  const committed = { id: 1, userData: { lockedTarget: target } };
  const free = { id: 2, userData: {} };
  assert.deepEqual(preserveLockedCombatants([committed, free], []), [committed]);
  assert.deepEqual(preserveLockedCombatants([committed, free], [free]), [committed, free]);
});
...
test("unpaired soldiers become neutral instead of roaming during battle", () => {
  ...
  assert.equal(soldierCombatState({combat:true,formingBattleLine:false,targetAlive:false,waitingSlot:true}),SOLDIER_COMBAT_STATE.WAITING);
  assert.equal(soldierCombatState({combat:true,formingBattleLine:true,targetAlive:false}),SOLDIER_COMBAT_STATE.FORMATION);
  assert.equal(soldierCombatState({combat:false,formingBattleLine:false,targetAlive:false,waitingSlot:true}),SOLDIER_COMBAT_STATE.FORMATION);
  assert.equal(soldierCombatState({combat:false,formingBattleLine:false,targetAlive:false}),SOLDIER_COMBAT_STATE.FORMATION);
});

test("free soldiers reserve at most one stay-around slot per active duel", () => {
  const duelA={id:"duel-a"},duelB={id:"duel-b"};
  const assignments=allocateDuelWaitingSlots(
    ["near-a","also-near-a","near-b"],
    [duelA,duelB],
    waiter=>waiter==="near-a"?duelA:null,
    (waiter,duel)=>waiter.endsWith(duel.id.at(-1))?1:10
  );
  assert.equal(assignments.get("near-a"),duelA);
  assert.equal(assignments.get("near-b"),duelB);
  assert.equal(assignments.has("also-near-a"),false);
  assert.equal(new Set(assignments.values()).size,assignments.size);
});

test("waiting soldiers hold well outside the active duel", () => {
  assert.ok(DUEL_WAITING_DISTANCE > 1.62 * 2);
});

test("free soldiers choose the closest available opponent pair", () => {
  const pair = chooseNearestAvailablePair(
    [{ id: "left-a", x: 0, z: 0 }, { id: "left-b", x: 8, z: 0 }],
    [{ id: "right-a", x: 6, z: 0 }, { id: "right-b", x: 1, z: 0 }],
...
test("threatened formations expand beyond double spacing and settle back", () => {
  const expanded = advanceFormationSpread(1, { threatDetected: true, dt: 1 });
  assert.ok(expanded > 2);
  assert.ok(expanded < THREAT_FORMATION_SCALE);
  const settled = advanceFormationSpread(THREAT_FORMATION_SCALE, { threatDetected: false, dt: 1 });
  assert.ok(settled > 1);
  assert.ok(settled < THREAT_FORMATION_SCALE);
});

test("formation expansion activates outer soldiers first and never adds rearward distance", () => {
  const outer=formationExpansionOffset(0,9,.35);
  const inner=formationExpansionOffset(4,9,.35);
  assert.ok(outer.activation>inner.activation);
  assert.ok(outer.forward>0);
  assert.equal(inner.forward,0);
  assert.ok(outer.forward>=0);
});
...
test("leaderless enemy packs center every soldier around one shared anchor", () => {
  for(const count of [3,4,5,6]){
    const offsets=Array.from({length:count},(_,index)=>centeredPackOffset(index,count));
    const lateralMean=offsets.reduce((sum,offset)=>sum+offset.lateral,0)/count;
    const forwardMean=offsets.reduce((sum,offset)=>sum+offset.forward,0)/count;
    assert.ok(Math.abs(lateralMean)<1e-9);
    assert.ok(Math.abs(forwardMean)<1e-9);
    assert.equal(new Set(offsets.map(offset=>`${offset.lateral},${offset.forward}`)).size,count);
  }
});
...
test("soldier chip health remains readable for the extended HUD duration", () => {
  assert.equal(SOLDIER_HEALTH_WIDGET_DURATION,3.2);
  const hit=advanceLaggingHealthBar({current:18,lag:25.6,hold:.24,visibleTimer:SOLDIER_HEALTH_WIDGET_DURATION,dt:.1});
