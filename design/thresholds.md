...
- Tactical command mode: clicking a living soldier selects its commander group; clicking a player commander selects only that commander. Selection applies a bright material lift plus an obvious 11% enlarged charcoal back-face ink contour that follows the full combat pose, plus a restrained ground halo, exposes a 7×7 destination grid, and slows simulation to 0.25× until a destination is issued or selection is cancelled. No bounding-box helper is used.
...
- Character facing: direction is communicated by a 0.035-unit charcoal frame drawn only around the four edges of the front face; there is no arrow or filled panel. Travel and commander flanking use movement-facing at 3.6 radians/second; committed soldier duels and commanders within 2.2 units of their opponent use opponent-facing at 4.6–5.2 radians/second. Corrections below 0.055 radians are ignored, and only one rotation source owns an actor per frame.
